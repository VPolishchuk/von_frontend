...
import useComplexesDetails from '../../../hook/useComplexesDetails';
// helpers
...
const LeftBox = (props) => (
  <div className='left-box'>
    <H3>{R.path(['complexe', 'name'], props)}</H3>
    {/* <Paragraph>
...
const SelectDateFilter = (props) => {
  const { optionsTypes } = useContext(GlobalContext);
...
const RightBox = (props) => (
  <div className='right-box'>
    <LocationPosition
      geo={R.path(['complexe', 'geo'], props)}
      email={R.path(['complexe', 'email'], props)}
      address={R.path(['complexe', 'address'], props)}
      location={R.path(['complexe', 'location'], props)}
    />
    <SelectDateFilter {...props} />
  </div>
);

export const ComplexesDetailSection = (props) => {
  const { query } = useRouter();

  const { complexe } = useComplexesDetails(query);
  const { complexes } = useComplexes(R.path(['location'], complexe));
  const isSelected = (item) => R.not(R.equals(item.id, complexe.id));
  const complexesList = complexes && R.map(
    (item) => item.guid,
    R.filter(isSelected, complexes)
  );
  // console.log('complexe___', complexe);
  return (
    <Wrapper>
      ...
      <Container className='container'>
        <LeftBox {...props} locationPlane={locationPlane} complexe={complexe}/>
        <RightBox {...props} complexe={complexe}/>
        {
          H.isNotNilAndNotEmpty(complexesList) &&
          <BottomBoxWrap>
            <H3>{`More locations in ${R.path(['location'], complexe)}`}</H3>
            <div>
              {/* change it data */}
              {
                H.isNotNilAndNotEmpty(complexesList) && locationPlane.moreLocations.map(
                  (item, i) => (
...
export default React.memo(ComplexesDetailSection);
